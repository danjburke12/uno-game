...
public class ForgotUNO extends Card{
    ...
    public int doAction(int currentPlayer) {
        ...
        System.out.println("\n Please select a number (Type " +Colors.getAnsiPurple() +"'-1'" +Colors.getAnsiReset() +" to cancel): ");
        selection = sc.nextInt(); //get selection
        
        //if option chosen is invalid...
        while ((selection < 1 && selection != -1) || selection > (Main.getMainGame().getPlayerCount())) {
        ...
        if (selection != -1) {
            ...
            System.out.println("Player chosen was " +p.getName());
